...
function popTerm(param) {
  ...
  self.url = '/ajax/get_suggest_word/';
  self.css = '/css/popterm2.css';
...
//    }
  ...
  self.update = function () {
    var keyword = encodeURIComponent(self.query);
    if (keyword == '') return;
    ...
    var url = this.url +
              '?keyword=' + keyword +
              '&suggest_limit=' + self.maxitems + r;
    self.send_suggest_request(url, null, self.callback, function() {});
    /*
  ...
  self.callback = function (res) {
    self.current = null;
    if (res.status !== 200 || res.suggest === null || res.suggest.length === 0) {
      self.list = [];
      self.f_construct ();
      self.f_hide();
    } else {
      self.list = res.suggest;
      self.f_construct();
      self.f_show();
    }
  };
  ...
  self.send_suggest_request = function send_suggest_request(url, data, success_event_handler, error_event_handler) {
    $.ajax({
      url: url,
      dataType: 'json',
      cache: false,
